...
Download [CS370\_Lab04.zip](src/CS370_Lab04.zip), saving it into the **CS370\_Fa26** directory.

Double-click on **CS370\_Lab04.zip** and extract the contents of the archive into a subdirectory called **CS370\_Lab04**

Open CLion, select **CS370\_Fa26** from the main screen (you may need to close any open projects), and open the **CMakeLists.txt** file in this directory (**not** the one in the **CS370\_Lab04** subdirectory). Uncomment the line
...
Download [CS370\_Lab04\_Solution.zip](sol/CS370_Lab04_Solution.zip), saving it into the **CS370\_Fa26** directory.
...
Open CLion, select **CS370\_Fa26** from the main screen (you may need to close any open projects), and open the **CMakeLists.txt** file in this directory (**not** the one in the **CS370\_Lab04\_Solution** subdirectory). Uncomment the line
...
- Add code in **callbacks.h** to **key\_callback()** to check for the following keys
...
- Add code in **spinningHexagon.cpp** to **main()** to register the *key\_callback* keyboard callback

- Add code in **spinningHexagon.cpp** to **render\_scene()** to create the translation matrix *trans_matrix* with offsets *hex\_x* and *hex\_y*. **Note:** Since we are in 2D, set the *z* offset to 0
...
- Add code in **callbacks.h** to **mouse\_callback()** to check for **GLFW\_MOUSE\_BUTTON\_LEFT** (left mouse button) being **GLFW\_PRESS** (pressed) and multiply *dir* by -1 (to flip the animation direction)

- Add code in **spinningHexagon.cpp** to **main()** to register the *mouse\_callback* mouse button callback
...
- Add code in **spinningHexagon.cpp** to **main()** to get the initial time and store the value in *elTime*

- Add code in **spinningHexagon.cpp** to **main()** (in the graphics loop) to get the current time and store the value in *curTime*

- Add code in **spinningHexagon.cpp** to **main()** (in the graphics loop) to update *hex\_angle* with a rate of *rpm* and the elapsed time calculated as *(curTime-elTime)*

- Add code in **spinningHexagon.cpp** to **main()** (in the graphics loop) to set *elTime* to *curTime* such that *dt* will be the time *between* rendering frames.

- Add code in **spinningHexagon.cpp** to **render\_scene()** to create the rotation matrix *rot_matrix* with angle *hex\_angle* and axis (0.0, 0.0, 1.0) (the *z* axis)
