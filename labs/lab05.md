...
Download [CS370\_Lab05.zip](src/CS370_Lab05.zip), saving it into the **CS370\_Fa26** directory.

Double-click on **CS370\_Lab05.zip** and extract the contents of the archive into a subdirectory called **CS370\_Lab05**

Open CLion, select **CS370\_Fa26** from the main screen (you may need to close any open projects), and open the **CMakeLists.txt** file in this directory (**not** the one in the **CS370\_Lab05** subdirectory). Uncomment the line
...
Download [CS370\_Lab05\_Solution.zip](sol/CS370_Lab05_Solution.zip), saving it into the **CS370\_Fa26** directory.
...
Open CLion, select **CS370\_Fa26** from the main screen (you may need to close any open projects), and open the **CMakeLists.txt** file in this directory (**not** the one in the **CS370\_Lab05\_Solution** subdirectory). Uncomment the line
...
- Add code in **orthoCube.cpp** to **main()** to *enable* the depth buffer via the **GL\_DEPTH\_TEST** flag.

- Add code in **orthoCube.cpp** to **main()** to *enable* back face culling via the **GL\_CULL\_FACE** flag.

- Add code in **orthoCube.cpp** to **display()** to also clear the depth buffer via the **GL\_DEPTH\_BUFFER\_BIT** flag (simply **\|**, i.e. *bitwise\-or*  the constants together in the call). What happens if you forget to do this?
...
- Add code in **orthoCube.cpp** to **display()** to set *proj\_matrix* to an *orthographic* projection with extents [-2.0,2.0] for all three axis.
...
- Add code in **orthoCube.cpp** to **main()** to get the initial window sizes storing the results in **ww** and **hh**

- Add code in **callbacks.h** to **framebuffer\_size\_callback()** to update **ww** and **hh** with the *width* and *height* parameters

- Add code in **orthoCube.cpp** to **main()** to register the *framebuffer\_size\_callback* resize callback. What happens if you forget to do this?

- Add code in **orthoCube.cpp** to **display()** to create an *anisotropic* orthographic projection matrix and store it in *proj_matrix*
