...
Download [CS370\_Lab09.zip](src/CS370_Lab09.zip), saving it into the **CS370\_Fa26** directory.

Double-click on **CS370\_Lab09.zip** and extract the contents of the archive into a subdirectory called **CS370\_Lab09**

Open CLion, select **CS370\_Fa26** from the main screen (you may need to close any open projects), and open the **CMakeLists.txt** file in this directory (**not** the one in the **CS370\_Lab09** subdirectory). Uncomment the line
...
Download [CS370\_Lab09\_Solution.zip](sol/CS370_Lab09_Solution.zip), saving it into the **CS370\_Fa26** directory.
...
Open CLion, select **CS370\_Fa26** from the main screen (you may need to close any open projects), and open the **CMakeLists.txt** file in this directory (**not** the one in the **CS370\_Lab09\_Solution** subdirectory). Uncomment the line
...
- Add code in **materials.h** to create a **MaterialProperties** variable named *brass* using the values given above.

- Add code in **materials.h** to **build\_materials()** to set the *brass* material in the **Materials** vector at the *Brass* index (be sure to do this *after* the *resize()* call)

- Add code in **materials.h** to add a new symbolic constant called **RedPlastic** to the **MaterialNames** enum *before* the *NumMaterials* element (note, this will allow the *NumMaterials* element to represent the number of materials being used).

- Add code in **materials.h** to create a **MaterialProperties** variable named *redPlastic* using (0.3f, 0.0f, 0.0f, 1.0f) for the ambient, (0.6f, 0.0f, 0.0f, 1.0f) for the diffuse, (0.8f, 0.6f, 0.6f, 1.0f) for the specular, and 32.0f for the shininess. **Note:** Don't forget to also add the pad to the structure.

- Add code in **materials.h** to **build\_materials()** to set the *redPlastic* material in the **Materials** vector at the *RedPlastic* index (be sure to do this *after* the *resize()* call)
...
- Add code in **lights.h** to create a **LightProperties** variable named *whiteLight* using the values given above.

- Add code in **lights.h** to **build\_lights()** to set the *whiteLight* light in the **Lights** vector at the *WhiteLight* index (be sure to do this *after* the *resize()* call).

- Add code in **lights.h** to **build\_lights()** to set the **WhiteLight** element of the **lightOn** array to 1, i.e. turning the light on by default.
...
- Add code in **gouraudMesh.cpp** to **render\_scene()** to compute the *normal\_matrix* variable as the transposed inverse of the model matrix for each object

- Add code in **gouraudMesh.cpp** to **render\_scene()** to draw the cube using the **draw\_mat\_object()** function by passing the *Cube* and *Brass* constants to the function. **Note:** In the shader, this index will select the corresponding material out of the materials buffer.

- Add code to **render\_scene()** to draw the sphere using the **draw\_mat\_object()** function by passing the *Sphere* and *RedPlastic* constants to the function. **Note:** In the shader, this index will select the corresponding material out of the materials buffer.

- Add code in **Gouraudlighting.vert** to **main()** to compute **l** as the *negative* of the light direction (i.e. direction *towards* the light) as a **vec3**

    ```cpp
    vec3 LightDirection = -normalize(vec3(Lights[0].direction));
    ```
    
- Add code in **Gouraudlighting.vert** to **main()** to compute **n** by transforming the vertex normal by the normal matrix

    ```cpp
    vec3 Normal = vec3(normalize(norm_matrix * normalize(vec4(vNormal,0.0f))));
    ```
 
- Add code in **Gouraudlighting.vert** to **main()** to compute **v** as the vector from the transformed surface location (in world coordinates) to the camera position (eye)
...
- Add code in **Gouraudlighting.vert** to **main()** to compute **h** as the (normalized) sum of the light direction and the view

    ```cpp
    vec3 HalfVector = normalize(LightDirection + View);
    ```

- Add code in **Gouraudlighting.vert** to **main()** to compute the *ambient* term

    ```cpp
    rgb += vec3(Lights[0].ambient*Materials[Material].ambient);
    ```

- Add code in **Gouraudlighting.vert** to **main()** to compute the *diffuse* term using Lambert's law
...
- Add code in **Gouraudlighting.vert** to **main()** to compute the *specular* term using the halfway vector and the shininess coefficient (only if there is a diffuse term)
