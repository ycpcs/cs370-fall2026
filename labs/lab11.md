...
Download [CS370\_Lab11.zip](src/CS370_Lab11.zip), saving it into the **CS370\_Fa26** directory.

Double-click on **CS370\_Lab11.zip** and extract the contents of the archive into a subdirectory called **CS370\_Lab11**

Open CLion, select **CS370\_Fa26** from the main screen (you may need to close any open projects), and open the **CMakeLists.txt** file in this directory (**not** the one in the **CS370\_Lab11** subdirectory). Uncomment the line
...
Download [CS370\_Lab11\_Solution.zip](sol/CS370_Lab11_Solution.zip), saving it into the **CS370\_Fa26** directory.
...
Open CLion, select **CS370\_Fa26** from the main screen (you may need to close any open projects), and open the **CMakeLists.txt** file in this directory (**not** the one in the **CS370\_Lab11\_Solution** subdirectory). Uncomment the line
...
- Add code in **materials.h** to append a new symbolic constant called **RedAcrylic** to the **MaterialNames** enum *before* the *NumMaterials* element (note, this will allow the *NumMaterials* element to represent the number of materials being used).

- Add code in **materials.h** to create a *redAcrylic* translucent material with alpha values 0.5f *for all three components*. **Hint:** Simply copy the *redPlastic* material as a starting point.

- Add code in **materials.h** to **build\_materials()** to set the *redAcrylic* material in the **Materials** vector at the *RedAcrylic* index (be sure to do this *after* the *resize()* call)

- Add code in **blendedMesh.cpp** to **render\_scene()** to draw the translucent cylinder using the **draw\_mat\_object()** function with the **Cone** model and the **RedAcrylic** material.
...
- Add code in **blendedMesh.cpp** to **main()** to enable alpha blending using **GL\_BLEND**

- Add code in **blendedMesh.cpp** to **main()** to set the blend factors to **GL\_SRC\_ALPHA** and **GL\_ONE\_MINUS\_SRC\_ALPHA**

- Add code in **Phonglighting.frag** to change the last component of *fragColor* from 1.0f to the material's ambient alpha value. **Note:** Without doing this blending will not occur because the fragment will always be opaque.
...
- Add code in **blendedMesh.cpp** to **render\_scene()** to disable depth buffer updating when rendering *only the translucent cone*. **Note:** Be sure to re-enable depth buffer updating after rendering the cone.

- Add code in **blendedMesh.cpp** to **main()** to animate the torus when the **roll\_torus** flag is true. **Hint:** Simply update the **torus\_theta** variable by **TORUS\_STEP** multiplied by **dT** to have the update time based. Then make sure the angle remains within the bounds of 0 to 360 degrees.
